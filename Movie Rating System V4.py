...
class GUI:
    ...
    def __init__(self, __parent, *args):
        ...
        search_label = Button(self.main_frame, text="Search", font=("Arial", "10"),
                              command = lambda:self.find_movie(self.movie_var))
        search_label.grid(row = 1, column = 5, columnspan=2, sticky = EW)

        # Genre selector
        self.genre_var = StringVar()
        self.genre_var.set("Any Genre")
        self.genre_options = ["Any Genre"]
        for movie in movies:
        ...
        # Rating Buttons
        like_button = Button(self.rate_frame, text="Like",
                             command = lambda: self.add_rating("5", CURRENT_MOVIE.id),
                             width = 32)
        like_button.grid(row=2, column=1, columnspan=3, )
        dislike_button = Button(self.rate_frame, text="Dislike",
                                command = lambda: self.add_rating("3", CURRENT_MOVIE.id),
                                width = 32)
        dislike_button.grid(row=2, column=4, columnspan=3)

        # Line Separator
        line_hor = Canvas(self.rate_frame, width=490, height=1, background="#ffc58f")
        line_hor.grid(row=3, column=0, columnspan=8, pady=10)
        line_vert = Canvas(self.rate_frame, width=1, height=200, background="#ffc58f")
        line_vert.grid(row=4, column=1, rowspan=6)

        # Recommendations labels
        recommendation_title = Label(self.rate_frame, text="Recommended Movies:",
                                     font=("Arial", "11", "bold"))
        recommendation_title.grid(row=4,column=2, columnspan=6)

        reco_movie_title = Label(self.rate_frame, text="Movie Title:",
                                 font=("Arial", "10", "bold"))
        reco_movie_title.grid(row=5, column=2, columnspan=3, pady=3)

        reco_movie_title = Label(self.rate_frame, text="Genres:",
                                 font=("Arial", "10", "bold"))
        reco_movie_title.grid(row=5, column=5, columnspan=3, pady=3)

        self.top_movies = Label(self.rate_frame, text="Shawshank Redemption, The \nRobots \nCars \nIsle of Dogs \nHarry Potter and the Philosopher's Stone",
                           font=("Arial", "10"))
        self.top_movies.grid(row=6, column=2, columnspan=3, sticky=EW)
        
        self.top_genres = Label(self.rate_frame, text="Crime|Drama \nAnimation|Children|Comedy|IMAX \nAnimation|Children|Comedy \nAnimation|Comedy|Adventure \nAdventure|Children|Fantasy",
                           font=("Arial", "10"))
        self.top_genres.grid(row=6, column=5, columnspan=3, sticky=EW)
    ...
    def display_movie(self, CURRENT_MOVIE):
        ...
        title_label.grid(row=0, column=0,columnspan=2, sticky=EW, pady = 5 )
        movie_title_label = Label(self.rate_frame, textvariable = movie_title_var, font=("Arial","10"))
        movie_title_label.grid(row=0, column=2, columnspan=4,sticky=EW)

        genre_label = Label(self.rate_frame, text = "Genres:", font=("Arial","10", "bold"))
        genre_label.grid(row=1, column=0, columnspan=2, sticky=EW, pady = 5)
        movie_genre_label = Label(self.rate_frame, textvariable = movie_genre_var, font=("Arial","10"))
        movie_genre_label.grid(row=1, column=2, columnspan=4,sticky=EW)
...
def genre_recommendations(sorted_movies, rated_movie, movie_rating):
    ...
    for movie in recommended_movie_object:
        ...
        rated_genres = (rated_movie_object.genres).split("|")
        for genre in genres:
        ...
        movie_genre_score = movie_genre_score/(len(genres))
        genre_score_dict.update({movie : (movie_genre_score/10)})

    return genre_score_dict
                         
def generate_recommendations(CURRENT_USER, num_of_recommendations, similar_users, rated_movie, movie_rating, chosen_genre):
    """
            ...
            if chosen_genre == "Any Genre":
                ...
                for movie in movies:
                    if key == movie.title:
                        ...
                        if chosen_genre in movie_genres:
                            movie_counter +=1
                            recommended_movies_title.append(key)
                            recommended_movies_genres.append("|".join(movie_genres))
